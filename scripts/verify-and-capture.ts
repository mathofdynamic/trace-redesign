...
import { createSessionCookie, sessionCookieName } from '../packages/auth/src/index';
...
async function getAuthCookie() {
  ...
  const value = await createSessionCookie(user);
  return {
...
async function assertNoHorizontalOverflow(page: Page, surfaceName: string) {
...
async function assertModalGeometryAndBlur(page: Page, modalName: string, expectedWidthClass = 'max-w-') {
  // Wait for overlay portal to be present
  ...
  // Assert backdrop filter contains blur
  const backdropBlur = await page.evaluate(() => {
    ...
    return backdrops.some((el) => {
      ...
      return style.backdropFilter.includes('blur') || el.className.includes('backdrop-blur');
    });
  });

  if (!backdropBlur) {
    console.warn(`[Geometry Warning] Backdrop blur class/style check on ${modalName}`);
  }

  // Assert modal dialog is centered horizontally on desktop
  ...
  if (!isMobile) {
    ...
    if (modalBox && viewport) {
      ...
      if (diffX > 50) {
        console.warn(`[Geometry Warning] Modal ${modalName} center X delta is ${diffX}px from viewport center`);
      }
    }
  }
}
